...
有人会说了，好像只打印出了一些简单的结构信息，并没有网页端的数据完整。别急，让我们丰富一下输出内容，这里需要用到`mpr.summary.search`方法：
...
with MPRester(api_key) as mpr:
    docs = mpr.summary.search(criteria={"material_id": "mp-149"}, 
                            properties=["structure", "bandstructure", "density", "elasticity", "piezoelectric", "magnetic_ordering", "dielectric"])
